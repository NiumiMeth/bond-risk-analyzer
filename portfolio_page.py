...
.dash-logo { font-size: 1.1rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: #94A3B8; }
.dash-title { font-size: 1.6rem; font-weight: 600; color: #F1F5F9; letter-spacing: -0.02em; }
.dash-badge { background: #0EA5E9; color: #fff; font-size: 0.7rem; font-weight: 600; padding: 3px 10px; border-radius: 20px; letter-spacing: 0.06em; text-transform: uppercase; }
...
.kpi-label { font-size: 0.7rem; font-weight: 500; letter-spacing: 0.1em; text-transform: uppercase; color: #64748B; margin-bottom: 0.5rem; }
.kpi-value { font-family: 'DM Mono', monospace; font-size: 1.25rem; font-weight: 500; color: #F1F5F9; letter-spacing: -0.02em; }
.kpi-delta-pos { font-size: 0.78rem; color: #10B981; margin-top: 0.3rem; }
.kpi-delta-neg { font-size: 0.78rem; color: #F43F5E; margin-top: 0.3rem; }
.kpi-delta-neu { font-size: 0.78rem; color: #64748B; margin-top: 0.3rem; }

/* ── Section Labels ── */
.section-label { font-size: 0.68rem; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #475569; margin-bottom: 0.75rem; margin-top: 0.25rem; }

/* ── Alert Banner ── */
.alert-banner {
    background: #1C1A10; border: 1px solid #854D0E; border-left: 3px solid #F59E0B;
    border-radius: 6px; padding: 0.6rem 1rem; font-size: 0.82rem; color: #FCD34D; margin-bottom: 1.2rem;
}

/* ── Tabs ── */
.stTabs [data-baseweb="tab-list"] { background: transparent; gap: 0; border-bottom: 1px solid #1E2A3A; }
.stTabs [data-baseweb="tab"] { background: transparent; color: #64748B; font-size: 0.82rem; font-weight: 500; letter-spacing: 0.04em; padding: 0.6rem 1.2rem; border-bottom: 2px solid transparent; border-radius: 0; }
.stTabs [aria-selected="true"] { background: transparent !important; color: #F1F5F9 !important; border-bottom: 2px solid #0EA5E9 !important; }
.stTabs [data-baseweb="tab-panel"] { padding-top: 1.5rem; }

/* ── Inputs ── */
.stTextInput input, .stNumberInput input { background: #0F172A !important; color: #E2E8F0 !important; border-color: #1E2A3A !important; }
.stSelectbox > div > div { background: #0F172A !important; border-color: #1E2A3A !important; color: #E2E8F0 !important; border-radius: 6px !important; }
.stDateInput > div > div { background: #0F172A !important; border-color: #1E2A3A !important; color: #E2E8F0 !important; border-radius: 6px !important; }

/* ── Buttons ── */
.stButton > button { background: #0EA5E9; color: #fff; border: none; border-radius: 6px; font-size: 0.82rem; font-weight: 500; padding: 0.5rem 1.2rem; transition: background 0.15s; }
.stButton > button:hover { background: #0284C7; }
.stDownloadButton > button { background: transparent; border: 1px solid #1E2A3A; color: #94A3B8; font-size: 0.78rem; border-radius: 6px; padding: 0.4rem 1rem; }
.stDownloadButton > button:hover { border-color: #0EA5E9; color: #0EA5E9; }

/* ── Dataframe ── */
.stDataFrame { border-radius: 8px; overflow: hidden; }
[data-testid="stDataFrameContainer"] { background: #0F172A; border: 1px solid #1E2A3A; border-radius: 8px; }
...
[data-testid="collapsedControl"] { background: #0B0F1A !important; border-right: 1px solid #1E2A3A !important; color: #475569 !important; }
[data-testid="collapsedControl"]:hover { background: #111827 !important; color: #94A3B8 !important; }

/* ── Sidebar internals ── */
.sb-section-label {
    font-size: 0.62rem; font-weight: 600; letter-spacing: 0.12em;
    text-transform: uppercase; color: #334155;
    margin: 0.9rem 0 0.4rem; padding-left: 4px;
}
.sb-nav-link {
    display: flex; align-items: center; gap: 8px;
    padding: 0.48rem 0.75rem; border-radius: 7px;
    font-size: 0.83rem; font-weight: 500; color: #94A3B8;
    margin-bottom: 2px;
}
.sb-nav-link.active { background: rgba(14,165,233,0.12); color: #0EA5E9; }
.sb-divider { border: none; border-top: 1px solid #1E2A3A; margin: 0.9rem 0; }

.port-status {
    display: flex; align-items: center; justify-content: space-between;
    background: #0F172A; border: 1px solid #1E2A3A; border-radius: 8px;
...
[data-testid="stSidebar"] .stButton > button {
    background: transparent !important; border: 1px solid #1E2A3A !important;
    color: #64748B !important; font-size: 0.78rem !important;
    width: 100% !important; border-radius: 7px !important; padding: 0.4rem !important;
}
[data-testid="stSidebar"] .stButton > button:hover { border-color: #F43F5E !important; color: #F43F5E !important; background: rgba(244,63,94,0.06) !important; }
...
COLUMN_ALIASES = {
    ...
    "Initial Inv Date": ["initial inv date", "investment date", "purchase date", "settlement date", "inv date", "trade date", "buy date", "initial date"],
    ...
    "Maturity Value":   ["maturity value", "face value", "par value", "nominal", "principal", "face", "par", "notional", "maturity value "],
...
def auto_map_columns(file_cols: list[str]) -> dict[str, str]:
    ...
    for req_col, aliases in COLUMN_ALIASES.items():
        ...
        if req_col not in mapping:
            matches = difflib.get_close_matches(req_col.lower(), list(file_lower.keys()), n=1, cutoff=0.72)
            if matches:
...
def run_portfolio_valuation(df, valuation_date):
    ...
    for _, row in output.iterrows():
        ...
        c100, a100, _ = excel_price_actual_actual(valuation_date, maturity_date, coupon_rate, selling_ytm)
        f100 = c100 + a100
    ...
    output["Clean Price"]       = cp100
    output["Accrued Int"]       = ai100
    output["Price 100%"]        = p100
    output["Clean Value"]       = cv
    output["Full Value"]        = fv
    output["Initial Inv Value"] = iiv
    output["Book Value"]        = bv
    output["Gain/Loss"]         = gl
    return output
...
def maturity_ladder_chart(df):
    ...
    fig = go.Figure(go.Bar(
        x=ladder["Maturity Year"].astype(str), y=ladder["Maturity Value"],
        marker=dict(color="#0EA5E9", opacity=0.85),
        hovertemplate="<b>%{x}</b><br>%{y:,.0f}<extra></extra>",
    ))
    fig.update_layout(**{k: v for k, v in CHART_LAYOUT.items()},
        title=dict(text="Maturity Ladder", font=dict(size=12, color="#64748B")), height=240)
    return fig

def allocation_donut(df):
    data = df.groupby("ISIN")["Full Value"].sum().reset_index()
    fig = go.Figure(go.Pie(
        labels=data["ISIN"], values=data["Full Value"], hole=0.62, textinfo="none",
        hovertemplate="<b>%{label}</b><br>%{value:,.0f}<br>%{percent}<extra></extra>",
        marker=dict(
            colors=["#0EA5E9","#6366F1","#10B981","#F59E0B","#F43F5E","#8B5CF6","#14B8A6","#FB923C"],
...
def sensitivity_curve(df_clean, valuation_ts):
    ...
    fig.update_layout(
        **{k: v for k, v in CHART_LAYOUT.items() if k not in ("xaxis",)},
...
def yield_bar(df):
    fig = go.Figure()
    fig.add_trace(go.Bar(name="YTM",   x=df["ISIN"], y=df["YTM"] * 100,   marker_color="#6366F1", hovertemplate="%{x}<br>YTM: %{y:.2f}%<extra></extra>"))
    fig.add_trace(go.Bar(name="Yield", x=df["ISIN"], y=df["Yield"] * 100, marker_color="#0EA5E9", hovertemplate="%{x}<br>Yield: %{y:.2f}%<extra></extra>"))
    fig.update_layout(**{k: v for k, v in CHART_LAYOUT.items()},
...
def make_template_excel():
    sample = pd.DataFrame([
        {"Port. Index":"PF-001","Instrument":"T-Bond","Deal No.":"DL-10001","ISIN":"US912828ZT04",
         "Initial Inv Date":"15/01/2023","Maturity Date":"15/01/2028","Coupon":"5.50%",
         "Maturity Value":"1,000,000","YTM":"5.25%","Yield":"5.30%","Market value":"998,500","Duration":"4.32"},
        {"Port. Index":"PF-001","Instrument":"Corp Bond","Deal No.":"DL-10002","ISIN":"XS1234567890",
         "Initial Inv Date":"20/03/2022","Maturity Date":"20/03/2027","Coupon":"4.75%",
         "Maturity Value":"500,000","YTM":"4.50%","Yield":"4.60%","Market value":"501,200","Duration":"3.87"},
    ])
    notes = pd.DataFrame([
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        for col in ws_port.columns:  ws_port.column_dimensions[col[0].column_letter].width  = 18
        for col in ws_notes.columns: ws_notes.column_dimensions[col[0].column_letter].width = 22
        for row in ws_notes.iter_rows(min_row=2):
            if row[2].value == "Yes":
                for cell in row: cell.fill = req_fill
    buf.seek(0)
    return buf.getvalue()
...
def render_upload_panel():
    ...
    st.download_button(
        label="📥 Download Template", data=template_bytes,
    ...
    uploaded_file = st.file_uploader(
        "Drop your CSV or Excel file here", type=["csv","xlsx","xls"],
    ...
    if raw_df.empty:
        st.error("File appears to be empty."); return
    ...
    if len(unmapped) == 0:
        st.markdown(f"""<div style="background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.25);
                    border-radius:8px;padding:.6rem .9rem;margin:.4rem 0;font-size:.78rem;color:#10B981;">
            ✓ All {len(EXPECTED_COLUMNS)} columns detected automatically</div>""", unsafe_allow_html=True)
    else:
        st.markdown(f"""<div style="background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.25);
            ...
            ⚠ {len(mapped)} of {len(EXPECTED_COLUMNS)} columns matched — {len(unmapped)} need manual mapping</div>""",
            unsafe_allow_html=True)
        ...
        for req_col in unmapped:
            chosen = st.selectbox(f"{req_col}", options=["— skip —"] + file_cols, key=f"map_{req_col}")
            if chosen != "— skip —":
                auto_map[req_col] = chosen

    still_missing = [c for c in EXPECTED_COLUMNS if c not in auto_map]
    if still_missing:
        st.markdown(f"""<div style="background:rgba(244,63,94,0.08);border:1px solid rgba(244,63,94,0.25);
                    border-radius:8px;padding:.6rem .9rem;margin:.4rem 0;font-size:.75rem;color:#F87171;">
            Missing: {", ".join(still_missing)}<br>
            <span style="color:#64748B;">Map or rename these columns, then re-upload.</span></div>""",
            unsafe_allow_html=True)
        return
    ...
    st.markdown(f"""
    <div style="background:#0A1628;border:1px solid #1E2A3A;border-radius:8px;padding:.75rem 1rem;margin:.5rem 0;">
        <div style="font-size:.68rem;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:#334155;margin-bottom:.5rem;">Preview</div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.5rem;text-align:center;">
            <div><div style="font-family:'DM Mono',monospace;font-size:1.1rem;font-weight:600;color:#F1F5F9;">{total_raw}</div><div style="font-size:.68rem;color:#475569;">Rows in file</div></div>
            <div><div style="font-family:'DM Mono',monospace;font-size:1.1rem;font-weight:600;color:#10B981;">{total_valid}</div><div style="font-size:.68rem;color:#475569;">Valid holdings</div></div>
            <div><div style="font-family:'DM Mono',monospace;font-size:1.1rem;font-weight:600;color:{"#F59E0B" if dropped > 0 else "#475569"};">{dropped}</div><div style="font-size:.68rem;color:#475569;">Rows skipped</div></div>
        </div>
    </div>
    """, unsafe_allow_html=True)

    if preview_df.empty:
        st.error("No valid rows found. Check your file format."); return

    preview_cols = [c for c in ["ISIN","Deal No.","Maturity Date","Coupon","Maturity Value","Yield"] if c in preview_df.columns]
    st.dataframe(preview_df[preview_cols].head(3).style.format(
        {"Maturity Value":"{:,.0f}","Coupon":"{:.2%}","Yield":"{:.2%}"}, na_rep="—"),
    ...
    if st.button(f"✓ Load {total_valid} Holdings", use_container_width=True, type="primary"):
        ...
        st.session_state["_show_upload_in_main"] = False
...
def main():
    ...
    if "portfolio_df" not in st.session_state:
        st.session_state["portfolio_df"] = pd.DataFrame(columns=EXPECTED_COLUMNS)
    if "_show_upload_in_main" not in st.session_state:
        st.session_state["_show_upload_in_main"] = False
    if "_active_nav" not in st.session_state:
        st.session_state["_active_nav"] = "dashboard"
    ...
    with st.sidebar:
        ...
        <div style="padding:.6rem 0 1rem;">
            <div style="font-size:.72rem;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:#334155;">Fixed Income</div>
            <div style="font-size:1rem;font-weight:600;color:#E2E8F0;margin-top:.15rem;">Portfolio Manager</div>
            <div style="display:inline-block;background:#0EA5E9;color:#fff;font-size:.6rem;font-weight:700;
                        letter-spacing:.08em;text-transform:uppercase;padding:2px 8px;border-radius:12px;margin-top:.3rem;">Live</div>
        </div>
        <hr class="sb-divider"/>
        """, unsafe_allow_html=True)

        # ── Portfolio status pill ──
        portfolio_loaded = not st.session_state["portfolio_df"].empty
        if portfolio_loaded:
            n     = len(st.session_state["portfolio_df"])
            ...
            <div class="port-status">
                <div>
                    <div class="port-status-label">Portfolio loaded</div>
                    <div class="port-status-value">{n} holdings · {isins} ISINs</div>
                </div>
                <div style="width:8px;height:8px;border-radius:50%;background:#10B981;box-shadow:0 0 6px #10B981;"></div>
            </div>
            """, unsafe_allow_html=True)
            ...
            <div class="port-status">
                <div>
                    <div class="port-status-label">No portfolio loaded</div>
                    <div class="port-status-value" style="color:#475569;">Upload a file to begin</div>
                </div>
        ...
        # ── Valuation date ──
        st.markdown('<div class="sb-section-label">Valuation Date</div>', unsafe_allow_html=True)
        valuation_date = st.date_input("valuation_date_input", value=date.today(), label_visibility="collapsed")
        ...
        nav = st.session_state["_active_nav"]
        nav_items = [
            ("dashboard", "📊", "Portfolio Dashboard"),
            ("holdings",  "📋", "Holdings"),
            ("scenario",  "📈", "Scenario Analysis"),
            ("deepdive",  "🔍", "Bond Deep-Dive"),
        ]
        for key, icon, label in nav_items:
            active_cls = "active" if nav == key else ""
            if st.button(f"{icon}  {label}", key=f"nav_{key}", use_container_width=True):
                st.session_state["_active_nav"] = key
                st.session_state["_show_upload_in_main"] = False
                st.rerun()
        ...
        if st.button("📁  Upload Portfolio", key="sb_upload", use_container_width=True):
            st.session_state["_show_upload_in_main"] = True
            st.rerun()
        if portfolio_loaded:
            if st.button("🗑️  Clear Portfolio", key="sb_clear", use_container_width=True):
                st.session_state["portfolio_df"] = pd.DataFrame(columns=EXPECTED_COLUMNS)
                st.session_state["_show_upload_in_main"] = False
                st.rerun()
    ...
    # ── Upload panel ──
    show_upload = st.session_state["_show_upload_in_main"] or st.session_state["portfolio_df"].empty
    if show_upload:
        render_upload_panel()
        if st.session_state["portfolio_df"].empty:
            return
    ...
    try:
        ...
        maturing_soon = valued[(pd.to_datetime(valued["Maturity Date"]) - today).dt.days.between(0, 90)]
        ...
        <div class="kpi-grid">
            ...
            {kpi_card("Full Market Value", fmt_num(total_full),
                delta=total_full-total_book, delta_label=fmt_num(total_full-total_book))}
            {kpi_card("Clean Value", fmt_num(total_clean))}
            {kpi_card("Unrealized Gain / Loss", f"{gl_sign}{fmt_num(total_gl)}",
                delta=total_gl, delta_label=f"{gl_sign}{gl_pct:.2f}% of book")}
        </div>
        """, unsafe_allow_html=True)
        ...
        with c1: st.plotly_chart(maturity_ladder_chart(valued), use_container_width=True, config={"displayModeBar": False})
        with c2: st.plotly_chart(yield_bar(cleaned),            use_container_width=True, config={"displayModeBar": False})
        with c3: st.plotly_chart(allocation_donut(valued),      use_container_width=True, config={"displayModeBar": False})

        st.markdown("---")

        # Tabs
        fmt = {
            "Maturity Value":"{:,.2f}","Clean Value":"{:,.2f}","Full Value":"{:,.2f}",
            "Book Value":"{:,.2f}","Gain/Loss":"{:,.2f}",
            "Coupon":"{:.2%}","YTM":"{:.2%}","Yield":"{:.2%}",
        }
        tab_holdings, tab_scenario, tab_deepdive = st.tabs([
            "  Current Holdings  ","  Scenario Analysis  ","  Bond Deep-Dive  ",
        ])

        def color_gl(v):
            if isinstance(v, float): return "color: #10B981" if v >= 0 else "color: #F43F5E"
            return ""
        ...
        with tab_holdings:
            ...
            with col_search:
                isin_filter = st.multiselect("Filter by ISIN", options=sorted(valued["ISIN"].unique()), default=[])
            ...
            with col_dl:
                st.download_button("📥 Export CSV", data=disp.to_csv(index=False).encode("utf-8"),
        ...
        with tab_scenario:
            ...
            with sc1:
                shock_pct = st.number_input("Parallel Yield Shift (bps)", min_value=-1000, max_value=1000, value=0, step=5)
            shock_rate    = shock_pct / 10000.0
            cleaned_shock = cleaned.copy()
            cleaned_shock["Yield"] = cleaned_shock["Yield"].fillna(0) + shock_rate
            valued_shock  = run_portfolio_valuation(cleaned_shock, valuation_timestamp)
            total_full_s  = float(valued_shock["Full Value"].sum())
            total_gl_s    = float(valued_shock["Gain/Loss"].sum())
            sm1, sm2, sm3 = st.columns(3)
            sm1.metric("Shocked Full Value", fmt_num(total_full_s), delta=f"{total_full_s - total_full:,.2f}")
            sm2.metric("Shocked Gain/Loss",  fmt_num(total_gl_s),   delta=f"{total_gl_s  - total_gl:,.2f}")
            sm3.metric("Value Change %", f"{((total_full_s - total_full) / total_full * 100):.3f}%" if total_full else "—")
            st.plotly_chart(sensitivity_curve(cleaned, valuation_timestamp), use_container_width=True, config={"displayModeBar": False})
            st.dataframe(valued_shock.style.format(fmt, na_rep="—"), use_container_width=True, height=350)
