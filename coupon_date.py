...
.section-label { font-size: 0.68rem; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #475569; margin-bottom: 0.5rem; }

/* Sidebar internals */
.sb-section-label { font-size: 0.62rem; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #334155; margin: .85rem 0 .35rem; padding-left: 4px; }
.sb-divider { border: none; border-top: 1px solid #1E2A3A; margin: .9rem 0; }
...
[data-testid="stSidebar"] .stButton > button {
    ...
    width: 100% !important; border-radius: 7px !important; padding: .4rem !important;
}
[data-testid="stSidebar"] .stButton > button:hover { border-color: #0EA5E9 !important; color: #0EA5E9 !important; }

/* Metric */
[data-testid="metric-container"] { background: transparent !important; }
[data-testid="metric-container"] label { color: #64748B !important; font-size: .72rem !important; letter-spacing: .06em !important; text-transform: uppercase !important; }
...
/* Inputs */
.stTextInput input, .stNumberInput input, .stSelectbox > div > div { background: #0F172A !important; border: 1px solid #1E2A3A !important; border-radius: 7px !important; color: #E2E8F0 !important; }
.stTextInput input:focus, .stNumberInput input:focus { border-color: #0EA5E9 !important; box-shadow: 0 0 0 3px rgba(14,165,233,.1) !important; }
label { color: #64748B !important; font-size: .78rem !important; }

/* Tabs */
.stTabs [data-baseweb="tab-list"] { background: transparent; border-bottom: 1px solid #1E2A3A; }
.stTabs [data-baseweb="tab"] { background: transparent; color: #64748B; font-size: .82rem; font-weight: 500; border-bottom: 2px solid transparent; border-radius: 0; }
...
/* Buttons */
.stButton > button { background: #0EA5E9; color: #fff; border: none; border-radius: 6px; font-size: .82rem; font-weight: 500; padding: .5rem 1.2rem; }
.stButton > button:hover { background: #0284C7; }

/* Panel card */
.dd-panel { background: #111827; border: 1px solid #1E2A3A; border-radius: 10px; padding: 1.25rem 1.5rem; margin-bottom: 1rem; position: relative; overflow: hidden; }
.dd-panel::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, #0EA5E9, #6366F1); }
.dd-panel-title { font-size: .68rem; font-weight: 600; letter-spacing: .12em; text-transform: uppercase; color: #475569; margin-bottom: .9rem; }

/* Coupon pill */
.cp-row { display: flex; align-items: center; justify-content: space-between; padding: .45rem 0; border-bottom: 1px solid #1E2A3A; font-size: .82rem; }
.cp-row:last-child { border-bottom: none; }
.cp-date { color: #94A3B8; font-family: 'DM Mono', monospace; font-size: .8rem; }
.cp-pill-paid { background: rgba(16,185,129,.12); color: #10B981; border-radius: 20px; padding: 2px 10px; font-size: .7rem; font-weight: 600; letter-spacing: .06em; text-transform: uppercase; }
.cp-pill-accruing { background: rgba(245,158,11,.12); color: #F59E0B; border-radius: 20px; padding: 2px 10px; font-size: .7rem; font-weight: 600; letter-spacing: .06em; text-transform: uppercase; }
.cp-pill-future { background: rgba(99,102,241,.12); color: #818CF8; border-radius: 20px; padding: 2px 10px; font-size: .7rem; font-weight: 600; letter-spacing: .06em; text-transform: uppercase; }
.cp-amount { color: #E2E8F0; font-family: 'DM Mono', monospace; font-size: .82rem; }

/* Info box */
.info-box { background: #0F172A; border: 1px solid #1E2A3A; border-left: 3px solid #0EA5E9; border-radius: 6px; padding: .6rem 1rem; font-size: .8rem; color: #94A3B8; margin-bottom: 1rem; }

/* PnL rows */
.pnl-row { display: flex; justify-content: space-between; padding: .5rem 0; border-bottom: 1px solid #1E2A3A; font-size: .85rem; }
...
def build_valuation_table(face_value, coupon_rate, annual_yield, maturity_date, valuation_date):
    ...
    if not coupon_dates:
        return pd.DataFrame(columns=["Cash Flow Date","Coupon CF","Principal CF","Total CF","Exponent (n + frac)","Discount Factor","PV"]), 0.0, 0.0, 0.0, 0.0
    next_coupon      = coupon_dates[0]
    days_to_next     = max((next_coupon - pd.Timestamp(valuation_date).normalize()).days, 0)
    frac             = days_to_next / 182.0
    ...
    for index, cash_date in enumerate(coupon_dates):
        ...
        dirty_price    += pv
        rows.append({"Cash Flow Date": cash_date.date(), "Coupon CF": coupon_cf, "Principal CF": principal_cf,
                     "Total CF": total_cf, "Exponent (n + frac)": exponent, "Discount Factor": discount_factor, "PV": pv})
    accrued_interest = max(0.0, min(period_coupon * (1.0 - frac), period_coupon))
...
def cashflow_timeline_chart(coupon_dates, face_value, coupon_rate, valuation_date):
    ...
    for d in coupon_dates:
        ...
        dates.append(d); amounts.append(amt)
        if d <= val_ts:
            colors.append("#10B981"); labels.append("Paid")
        else:
    ...
    for d, a, c, l in zip(dates, amounts, colors, labels):
        fig.add_trace(go.Bar(x=[d], y=[a], marker_color=c, name=l,
            hovertemplate=f"<b>{d.date()}</b><br>{l}<br>{a:,.2f}<extra></extra>", showlegend=False))
    fig.add_vline(x=val_ts.timestamp() * 1000, line_color="#F59E0B", line_dash="dash", line_width=1.5,
...
def pv_bar_chart(calc_table):
    if calc_table.empty: return None
    fig = go.Figure(go.Bar(
        x=calc_table["Cash Flow Date"].astype(str), y=calc_table["PV"],
        marker=dict(color=calc_table["PV"], colorscale=[[0,"#1E2A3A"],[1,"#0EA5E9"]], showscale=False),
        hovertemplate="<b>%{x}</b><br>PV: %{y:,.2f}<extra></extra>",
    ))
    fig.update_layout(**{k: v for k, v in CHART_LAYOUT.items() if k != "legend"},
        title=dict(text="Present Value per Cash Flow", font=dict(size=12, color="#64748B")), height=220)
    return fig
...
def show_deep_dive(selected_row: pd.Series, valuation_timestamp: pd.Timestamp) -> None:
    ...
    <div class="dd-panel">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;">
            <div>
                <div style="font-size:.68rem;color:#475569;text-transform:uppercase;letter-spacing:.1em;margin-bottom:.2rem;">ISIN</div>
                <div style="font-family:'DM Mono',monospace;color:#E2E8F0;font-size:.9rem;">{isin}</div>
            </div>
            <div>
                <div style="font-size:.68rem;color:#475569;text-transform:uppercase;letter-spacing:.1em;margin-bottom:.2rem;">Deal No.</div>
                <div style="font-family:'DM Mono',monospace;color:#E2E8F0;font-size:.9rem;">{deal}</div>
            </div>
            <div>
                <div style="font-size:.68rem;color:#475569;text-transform:uppercase;letter-spacing:.1em;margin-bottom:.2rem;">Instrument</div>
                <div style="color:#94A3B8;font-size:.9rem;">{instr or "—"}</div>
            </div>
            <div>
                <div style="font-size:.68rem;color:#475569;text-transform:uppercase;letter-spacing:.1em;margin-bottom:.2rem;">Investment Date</div>
    ...
    tab_val, tab_schedule, tab_pnl = st.tabs(["  Valuation  ","  Coupon Schedule  ","  P&L Summary  "])

    # ── TAB 1: Valuation ─────────────────────────────────────────────────────
    with tab_val:
        vc1, vc2 = st.columns([1, 2])
        with vc1:
            y_input     = st.number_input("Base Yield (%)", value=round(base_yield * 100.0, 4), format="%.4f")
        ...
        with ch1:
            st.plotly_chart(cashflow_timeline_chart(schedule, face, coupon, val_ts), use_container_width=True, config={"displayModeBar": False})
        with ch2:
            pvc = pv_bar_chart(calc_table)
            if pvc: st.plotly_chart(pvc, use_container_width=True, config={"displayModeBar": False})

        st.markdown('<div class="section-label" style="margin-top:1rem;">Cash-Flow Table</div>', unsafe_allow_html=True)
        fmt_cf = {"Coupon CF":"{:,.2f}","Principal CF":"{:,.2f}","Total CF":"{:,.2f}",
                  "Exponent (n + frac)":"{:.12f}","Discount Factor":"{:.12f}","PV":"{:.6f}"}
        st.dataframe(calc_table.style.format(fmt_cf, na_rep="—"), use_container_width=True, height=300)
    ...
    with tab_schedule:
        ...
        st.markdown('<div class="info-box">This is a <strong>fixed-rate bond</strong> — coupon cash flow is identical every period (<code>Face × Rate ÷ 2</code>). The <strong>PV</strong> column shows discounted present value per payment.</div>', unsafe_allow_html=True)
        ...
        for d in schedule:
            ...
            if is_paid:         pill = '<span class="cp-pill-paid">Paid</span>'
            elif is_accruing:   pill = '<span class="cp-pill-accruing">Accruing</span>'
            else:               pill = '<span class="cp-pill-future">Upcoming</span>'
        ...
        st.markdown(f'<div class="dd-panel"><div class="dd-panel-title">Full Coupon Schedule</div>{rows_html}</div>', unsafe_allow_html=True)
    ...
    with tab_pnl:
        ...
        with fc1:
            funding_rate_local = st.number_input("Funding Rate (%)", value=0.0, step=0.01, format="%.4f")
...
if __name__ == "__main__":
    st.set_page_config(
        page_title="Bond Deep Dive", layout="wide",
    ...
    with st.sidebar:
        ...
        <div style="padding:.5rem 0 .8rem;">
            ...
            <div style="font-size:1rem;font-weight:600;color:#E2E8F0;margin-top:.15rem;">Bond Deep-Dive</div>
        </div>
        <hr class="sb-divider"/>
        """, unsafe_allow_html=True)

        st.markdown('<div class="sb-section-label">Valuation Date</div>', unsafe_allow_html=True)
        valuation_date = st.date_input("valuation_deepdive", value=date.today(), label_visibility="collapsed")

        # Bond selector — if session state has a bond pre-selected
        if "selected_bond" in st.session_state:
            bond = st.session_state["selected_bond"]
            isin = bond.get("ISIN", "Unknown")
            deal = bond.get("Deal No.", "Unknown")
            st.markdown(f"""
            <hr class="sb-divider"/>
            <div class="sb-section-label">Selected Bond</div>
            <div style="background:#0F172A;border:1px solid #1E2A3A;border-radius:8px;padding:.6rem .9rem;margin-top:.3rem;">
                <div style="font-size:.68rem;color:#475569;">ISIN</div>
                <div style="font-family:'DM Mono',monospace;color:#E2E8F0;font-size:.85rem;">{isin}</div>
                <div style="font-size:.68rem;color:#475569;margin-top:.4rem;">Deal No.</div>
                <div style="font-family:'DM Mono',monospace;color:#94A3B8;font-size:.85rem;">{deal}</div>
            </div>
            """, unsafe_allow_html=True)
            if st.button("✕  Clear Selection", key="clear_bond"):
                del st.session_state["selected_bond"]
                st.rerun()

    # Main content
    if "selected_bond" in st.session_state:
        show_deep_dive(st.session_state["selected_bond"], pd.Timestamp(valuation_date))
    else:
        st.markdown("""
        <div style="text-align:center;padding:5rem 2rem;color:#475569;">
            ...
            <div style="font-size:.82rem;margin-top:.4rem;">
                Use the Portfolio Manager to open a deep-dive, or set <code>st.session_state["selected_bond"]</code> directly.
            </div>
        </div>
        """, unsafe_allow_html=True)